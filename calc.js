...
const appxButton =document.getElementById('approximatingButton')
 ...
 function toggleCalculator(){
  if(!calculatorOn){
   ...
   output.classList.remove('answerFieldPopUp');
			...
			homeButton.removeAttribute("disabled","disabled");
			appxButton.removeAttribute("disabled","disabled");
   output.value = '';
			exponentField.value = "";
   inResult.value = '';
...
function calculate(){
   ...
   if(/^\d+$/.test(inputElement.value) && parseInt(inputElement.value) >= "0"){
        ...
        .replace(/π/g, Math.PI)
        .replace(/³√/g, 'cbrt')
								.replace(/'sin($1)'/g,'sin($1Math.PI/180)')
        .replace(/√/g, 'sqrt')
								.replace(/²/g, '^2')
								.replace(/³/g, '^3')
      ...
      const result = math.evaluate(expressionWithReplacements);
						...
						if (!isNaN(result)){
							...
							? resultStr.split('e')
       : [resultStr, '0'];
							const formattedResult = decimalPlaces !== null ? roundToDecimalPlaces(mantissa, decimalPlaces) : mantissa;
							answerField.textContent = formattedResult;	
...
appxButton.addEventListener('click', () => {
	const result = parseFloat(answerField.textContent);
	if (decimalPlaces === null) {
		answerField.textContent = roundToDecimalPlaces(result, 2) ;
 }
});
...
function updateExpressionAndAnswer() {
	const { expression, result } = history[currentIndex];
	inputElement.value = expression;
	answerField.textContent = `Answer: ${result}`;
}
function updateHistoryList() {
	historyList.innerHTML = "";
	history.forEach((item) => {
					const li = document.createElement("li");
					li.textContent = `Expression: ${item.expression} => Result: ${item.result}`;
...
upButton.addEventListener("click", () => {
	if (history.length === 0){
		...
		inputElement.value = "NO HISTORY AVAILABLE!...";
		answerField.textContent = '';
		return;
	}
	 updateExpressionAndAnswer()
...
function updateExpressionAndAnswer() {
	const { expression, result } = history[currentIndex];
	inputElement.value = expression;
	const { mantissa, exponent } = formatResult(result);
